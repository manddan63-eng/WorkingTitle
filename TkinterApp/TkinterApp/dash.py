...
class App:
    def __init__(self, root):
        ...
        print('---->')
        print(self.create_dashboard_var.get())
    ...
    def generate_weekly_dashboard_2025(self, journal_path):
        try:
            ...
            #топчик по филиалам
            branch_mapping = {'Ф': 'Северный','ФСВ': 'Северо-Восточный','ФСЗ': 'Северо-Западный','ФЮ': 'Южный','ФЮВ': 'Юго-Восточный','ФЮЗ': 'Юго-Западный','ФВ': 'Восточный','ФЗ': 'Западный','ФС(Э)': 'Северный','ФСВ(Э)': 'Северо-Восточный','ФСЗ(Э)': 'Северо-Западный','ФЮ(Э)': 'Южный','ФЮВ(Э)': 'Юго-Восточный','ФЮЗ(Э)': 'Юго-Западный','ФВ(Э)': 'Восточный','ФЗ(Э)': 'Западный','ФС (Э)': 'Северный','ФСВ (Э)': 'Северо-Восточный','ФСЗ (Э)': 'Северо-Западный','ФЮ (Э)': 'Южный','ФЮВ (Э)': 'Юго-Восточный','ФЮЗ (Э)': 'Юго-Западный','ФВ (Э)': 'Восточный','ФЗ (Э)': 'Западный','ФС(э)': 'Северный','ФСВ(э)': 'Северо-Восточный','ФСЗ(э)': 'Северо-Западный','ФЮ(э)': 'Южный','ФЮВ(э)': 'Юго-Восточный','ФЮЗ(э)': 'Юго-Западный','ФВ(э)': 'Восточный','ФЗ(э)': 'Западный','ФС (э)': 'Северный','ФСВ (э)': 'Северо-Восточный','ФСЗ (э)': 'Северо-Западный','ФЮ (э)': 'Южный','ФЮВ (э)': 'Юго-Восточный','ФЮЗ (э)': 'Юго-Западный','ФВ (э)': 'Восточный','ФЗ (э)': 'Западный','ФЦ':'Центральный','ФЦ (э)':'Центральный','ФЦ(э)':'Центральный','ФЦ (Э)':'Центральный','ФЦ(Э)':'Центральный'}	

            top_branches=df.copy()

            top_branches['Название филиала'] = top_branches['Название филиала'].map(branch_mapping)

            top_branches = top_branches['Название филиала'].value_counts().head(10)

            branch_labels = top_branches.index.tolist()
            branch_values = top_branches.values.tolist()
            branch_labels_js = json.dumps(branch_labels, ensure_ascii=False)
            branch_values_js = json.dumps(branch_values)


            #listener на обновление файла Журнал?


            df_serial = df.copy()
            df_serial['Дата ДТП'] = df_serial['Дата ДТП'].dt.strftime('%Y-%m-%d')
            districts = sorted(df['Район'].dropna().unique())
            causes = sorted(df['Причина ДТП'].dropna().unique())
            
            
            
            # Агрегация
            # Создаем словарь chart_data для хранения агрегированных данных для графиков.
            chart_data = {
                # Подсчет топ-10 причин ДТП.
                'causes': df['Причина ДТП'].value_counts().head(10).to_dict(),
                # Подсчет топ-10 районов с ДТП.
                'districts': df['Район'].value_counts().head(10).to_dict(),
                # Подсчет количества ДТП по округам.
                'okrugs': df['Округ'].value_counts().to_dict()
            }            
            ...
            html_content = f'''
                ...
                <div class="container">
                  <h1>Топ филиалов по ДТП (2025–2026)</h1>
                  <div id="chart_branches" class="overallStyle"></div>
                </div>

                <script>
                // === ДАННЫЕ ДЛЯ СТАТИЧНЫХ ГРАФИКОВ ===
                const weeks1 = {json.dumps(weeks1)};
                const total1 = {json.dumps(total1)};
                const carrier1 = {json.dumps(carrier1)};
                const rest1 = {json.dumps(rest1)};

                const weeks2 = {json.dumps(weeks2)};
                const total2 = {json.dumps(total2)};
                const carrier2 = {json.dumps(carrier2)};
                const rest2 = {json.dumps(rest2)};

                const weeks3 = {json.dumps(weeks3)};
                const total3 = {json.dumps(total3)};
                const carrier3 = {json.dumps(carrier3)};
                const rest3 = {json.dumps(rest3)};

                const weeks1_2026 = {json.dumps(weeks1_2026)};
                const total1_2026 = {json.dumps(total1_2026)};
                const carrier1_2026 = {json.dumps(carrier1_2026)};
                const rest1_2026 = {json.dumps(rest1_2026)};

                const weeks2_2026 = {json.dumps(weeks2_2026)};
                const total2_2026 = {json.dumps(total2_2026)};
                const carrier2_2026 = {json.dumps(carrier2_2026)};
                const rest2_2026 = {json.dumps(rest2_2026)};

                const weeks3_2026 = {json.dumps(weeks3_2026)};
                const total3_2026 = {json.dumps(total3_2026)};
                const carrier3_2026 = {json.dumps(carrier3_2026)};
                const rest3_2026 = {json.dumps(rest3_2026)};

                const branchLabels = {branch_labels_js};
                const branchValues = {branch_values_js};
                ...
                Plotly.newPlot('chart', [
                  ...
                  {{
                    ...
                    text: total1,
                    customdata: total1,
                    hovertemplate: '2025. Всего ДТП: %{{customdata}}'
                  }},
                  {{
                    x: x_2026_1,
                    y: carrier1_2026,
                  ...
                  {{
                    ...
                    hovertemplate: '2026. Всего ДТП: %{{customdata}}'
                  }}
                ], {{
                  title: 'ДТП по неделям',
                  xaxis: {{ title: 'Неделя года', tickmode: 'array', tickvals: Array.from({{length: 53}}, (_, i) => i + 1) }},
                ...
                Plotly.newPlot('chart2', [
                  {{ x: x_2025_2, y: carrier2, type: 'bar', name: '2025: По вине перевозчика', marker: {{ color: '#C73E1D' }}, text: carrier2, textposition: 'inside', textfont: {{ color: 'white' }}, hovertemplate: 'По вине: %{{y}}' }},
                  {{ x: x_2025_2, y: rest2, type: 'bar', name: '2025: Остальные', marker: {{ color: '#2E86AB' }},text: total2,customdata: total2,hovertemplate: '2025. Всего ДТП: %{{customdata}}' }},
                  {{ x: x_2026_2, y: carrier2_2026, type: 'bar', name: '2026: По вине перевозчика', marker: {{ color: '#A52A2A' }}, text: carrier2_2026, textposition: 'inside', textfont: {{ color: 'white' }}, hovertemplate: 'По вине: %{{y}}' }},
                  {{ x: x_2026_2, y: rest2_2026, type: 'bar', name: '2026: Остальные', marker: {{ color: '#1E5E8C' }}, text: total2_2026,customdata: total2_2026,hovertemplate: '2026. Всего ДТП: %{{customdata}}'  }}
                ], {{
                ...
                Plotly.newPlot('chart3', [
                  {{ x: x_2025_3, y: carrier3, type: 'bar', name: '2025: По вине перевозчика', marker: {{ color: '#C73E1D' }}, text: carrier3, textposition: 'inside', textfont: {{ color: 'white' }}, hovertemplate: 'По вине: %{{y}}' }},
                  {{ x: x_2025_3, y: rest3, type: 'bar', name: '2025: Остальные', marker: {{ color: '#2E86AB' }}, text: total3,customdata: total3,hovertemplate: '2025. Всего ДТП: %{{customdata}}'  }},
                  {{ x: x_2026_3, y: carrier3_2026, type: 'bar', name: '2026: По вине перевозчика', marker: {{ color: '#A52A2A' }}, text: carrier3_2026, textposition: 'inside', textfont: {{ color: 'white' }}, hovertemplate: 'По вине: %{{y}}' }},
                  {{ x: x_2026_3, y: rest3_2026, type: 'bar', name: '2026: Остальные', marker: {{ color: '#1E5E8C' }}, text: total3_2026,customdata: total3_2026,hovertemplate: '2026. Всего ДТП: %{{customdata}}' }}
                ], {{
                ...
                Plotly.newPlot('chart_branches', [{{
                  ...
                  title: 'Топ филиалов по количеству ДТП',
                  xaxis: {{ title: 'Количество ДТП' }},
                  yaxis: {{ title: 'Филиал', autorange: 'reversed' }},
                  margin: {{ l: 150, r: 50, t: 50, b: 30 }}
                }});
                ...
                document.addEventListener('DOMContentLoaded', () => {{
                  ...
                  // Заполняем фильтры
                  rawData.filters.districts.forEach(d => {{
                  ...
                  rawData.filters.causes.forEach(c => {{
                    const opt = document.createElement('option');
                    opt.value = c;
                    opt.textContent = c;
                ...
                function filterData() {{
                  ...
                  toDate.setDate(toDate.getDate() + 1); // включительно

                  const district = document.getElementById('district-filter').value;
                  const cause = document.getElementById('cause-filter').value;

                  return rawData.records.filter(row => {{
                    const rowDate = new Date(row['Дата ДТП']);
                ...
                function updateCharts() {{
                  ...
                  if (causes.length > 0) {{
                    Plotly.newPlot(container.appendChild(document.createElement('div')), [{{
                      y: causes.map(d => d[0]),
                      x: causes.map(d => d[1]),
                      type: 'bar',
                      orientation: 'h',
                      marker: {{ color: '#C73E1D' }}
                    }}], {{ title: 'Топ причин ДТП', margin: {{ t: 40, l: 150 }} }});
                  }}
                  ...
                  if (districts.length > 0) {{
                    Plotly.newPlot(container.appendChild(document.createElement('div')), [{{
                      y: districts.map(d => d[0]),
                      x: districts.map(d => d[1]),
                      type: 'bar',
                      orientation: 'h',
                      marker: {{ color: '#3BB273' }}
                    }}], {{ title: 'Топ районов', margin: {{ t: 40, l: 150 }} }});
                  }}
                  ...
                  if (okrugs.length > 0) {{
                    Plotly.newPlot(container.appendChild(document.createElement('div')), [{{
                      ...
                      type: 'bar',
                      marker: {{ color: '#5D5D81' }}
                    }}], {{ title: 'ДТП по округам', margin: {{ t: 40, b: 80 }} }});
                  }}
                }}
                </script>
                </body>
            ...
            output_html = journal_path.replace('.xlsx', '_dashboard_2025_2026_weekly.html')
